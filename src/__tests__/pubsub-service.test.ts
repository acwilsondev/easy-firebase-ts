...
jest.mock('@google-cloud/pubsub', () => {
  ...
  // Mock Message class
  class Message {
    constructor(public data: Buffer, public attributes?: Record<string, string>) {}
    ack(): void {}
  }
  ...
  Topic.prototype.createSubscription = jest.fn().mockImplementation(
    (name: string) => Promise.resolve([new Subscription(name)])
  );
...
describe('PubSubService', () => {
  ...
  describe('publishMessage', () => {
    ...
    it('should throw PubSubError when publish fails', async () => {
      ...
      await expect(pubSubService.publishMessage('test-topic', 'test-message'))
  ...
  describe('subscribeToTopic', () => {
    ...
    it('should throw PubSubError when subscription creation fails', async () => {
      ...
      const { Topic } = require('@google-cloud/pubsub');
      Topic.prototype.createSubscription = jest.fn().mockRejectedValue(new Error('Subscription error'));
      
      const handleMessage = jest.fn();
      await expect(pubSubService.subscribeToTopic('test-subscription', 'test-topic', handleMessage))
        .rejects.toThrow(PubSubError);
    });
  });
